...
void sched_init_numa(void)
{
	...
	int nr_levels = 0;
	int i, j;
	int nr_usable_node_ids = min_t(int, nr_node_ids, 2);
	...
	for (i = 0; i < nr_usable_node_ids; i++) {
		for (j = 0; j < nr_usable_node_ids; j++) {
	...
	for (i = 0; i < nr_levels; i++) {
		sched_domains_numa_masks[i] =
			kzalloc(nr_usable_node_ids * sizeof(void *), GFP_KERNEL);
	...
	sched_numa_onlined_nodes = bitmap_alloc(nr_usable_node_ids, GFP_KERNEL);
	if (!sched_numa_onlined_nodes)
		return;

	bitmap_zero(sched_numa_onlined_nodes, nr_usable_node_ids);
