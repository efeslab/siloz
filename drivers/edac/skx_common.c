...
// kevlough: hardcode some SKX addresses and devices
#define MAGIC_BUSSES_BASE 0x80000000
#define TOLM_TOHM_BUS 0x17
#define TOLM_TOHM_DEV 0x05
#define TOLM_TOHM_FUNC 0x0
#define UBOX_SLOT 0x08
#define UBOX_FUNC 0x2

static const u8 MAGIC_BUSSES[8] = {0x00, 0x17, 0x3a, 0x5d, 0x80, 0x85, 0xae, 0xd7};
static const u8 UBOX_BUSSES[2] = {0x0, 0x80};
...
skx_decode_f skx_decode = NULL;
static skx_show_retry_log_f skx_show_retry_rd_err_log;
u64 skx_tolm = 0;
u64 skx_tohm = 0;
EXPORT_SYMBOL_GPL(skx_tolm);
EXPORT_SYMBOL_GPL(skx_tohm);
static LIST_HEAD(dev_edac_list);
static bool skx_mem_cfg_2lm;

int early_read_pci_config(u8 bus, u8 dev, u8 func, u8 off, u32 *reg)
{
	*reg = read_pci_config(bus, dev, func, off);
	if (*reg == (u32) -1) {
		printk(KERN_INFO "early_skx: bus error for bus 0x%x, dev 0x%x, func 0x%x, off 0x%x\n", bus, dev, func, off);
		return 1;
	}
	return 0;
}
...
/**
 * kevlough: need to read info before main Linux PCI system is initiliazed.
 * use "early_" functions accordingly
 * 
...
int __init early_init_read_pci_config(u8 bus, u8 dev, u8 func, u32 off, u32 *reg)
{
	...
	if (off < 256) {
		...
		for (i = 0; i < sizeof(MAGIC_BUSSES) / sizeof(MAGIC_BUSSES[0]); i++) {
			if (bus == MAGIC_BUSSES[i]) {
				// kevlough: see https://wiki.osdev.org/PCI_Express#Enhanced_Configuration_Mechanism
				addr = MAGIC_BUSSES_BASE + ((((u64) bus) << 20) | (((u64) dev) << 15) | (((u64) func) << 12));
				io_addr = early_ioremap(addr, 4096);
				addr = (u64) io_addr;
				*reg = readl((void __iomem *) (addr + off));
				if (*reg == (u32) -1) {
					printk(KERN_INFO "early_skx: bus error for bus 0x%x, dev 0x%x, func 0x%x, off 0x%x\n", bus, dev, func, off);
					early_iounmap(io_addr, 4096);
					return 1;
				}
				early_iounmap(io_addr, 4096);
				return 0;
		...
		printk(KERN_INFO "early_skx: failed to find bus 0x%x\n", bus);
		return 1;
	}
}
...
int early_skx_get_src_id(struct skx_dev *d, int off, u8 *id)
{
	...
	if (early_read_pci_config(d->util_all_bus, d->util_all_dev, d->util_all_func, off, &reg)) {
		printk(KERN_INFO "early_skx: Invalid bdf for get_src_id\n");
		return 1;
	}
...
int early_skx_get_node_id(struct skx_dev *d, u8 *id)
{
	...
	if (early_read_pci_config(d->util_all_bus, d->util_all_dev, d->util_all_func, 0xf4, &reg)) {
		printk(KERN_INFO "early_skx: Invalid bdf for get_node_id\n");
		return 1;
	}
 ...
 * and build the full list of all the skx_dev structures that we need here.
 * 
 * kevlough: use this version during early boot
 */
int early_skx_get_all_bus_mappings(unsigned int did, int off, enum type type,
			     struct skx_dev *skx_sock_devs)
{
	struct skx_dev *d;
	u32 reg;
	int ndev;

	// kevlough: hardcoded max 2 sockets
	for (ndev = 0; ndev < sizeof(UBOX_BUSSES) / sizeof(UBOX_BUSSES[0]); ndev++) {
		d = skx_sock_devs + ndev;
		if (early_read_pci_config(UBOX_BUSSES[ndev], UBOX_SLOT, UBOX_FUNC, off, &reg)) {
			printk(KERN_INFO "early_skx: invalid bdf for early_skx_get_all_bus_mappings\n");
			return 1;
		}
...
/**
 * kevlough: get tolm (top-of-low-memory) and tohm (top-of-high-memory)
 * during early boot
*/
int early_skx_get_hi_lo(unsigned int did, int off[], u64 *tolm, u64 *tohm)
{
	...
	if (early_read_pci_config(TOLM_TOHM_BUS, TOLM_TOHM_DEV, TOLM_TOHM_FUNC, off[0], &reg)) {
		printk(KERN_INFO "early_skx: Invalid TOLM BDF\n");
		return 1;
	}
	skx_tolm = reg;
	if (early_read_pci_config(TOLM_TOHM_BUS, TOLM_TOHM_DEV, TOLM_TOHM_FUNC, off[1], &reg)) {
		printk(KERN_INFO "early_skx: Invalid TOHM lo BDF\n");
		return 1;
	}
	skx_tohm = reg;
	if (early_read_pci_config(TOLM_TOHM_BUS, TOLM_TOHM_DEV, TOLM_TOHM_FUNC, off[2], &reg)) {
		printk(KERN_INFO "early_skx: Invalid TOHM hi BDF\n");
		return 1;
	}
	skx_tohm |= (u64)reg << 32;
	...
	*tohm = skx_tohm;
	...
	if (skx_tolm == 0) {
		printk(KERN_INFO "early_skx: Invalid TOLM (0x0)\n");
		return 1;
	}

	if (skx_tohm == 0) {
		printk(KERN_INFO "early_skx: Invalid TOHM (0x0)\n");
		return 1;
	}
	return 0;
}
		      ...
		      struct skx_imc *imc, int chan, int dimmno)
{
	...
	imc->close_pg = GET_BITFIELD(mcmtr, 0, 0);
	imc->bank_xor_enable = GET_BITFIELD(mcmtr, 9, 9);
	imc->fine_grain_bank = GET_BITFIELD(amap, 0, 0);
	imc->rowbits = rows;
	imc->colbits = cols;
