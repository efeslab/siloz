 ...
 * 
 * kevlough: this is the original version of the function, which
 * should only run prior to Siloz iniitialization completing to
 * ensure correct states for all logical nodes.
 */
static int refresh_cpu_vm_stats_no_siloz(bool do_pagesets)
{
	struct pglist_data *pgdat;
	struct zone *zone;
	int i;
 ...
 * The function returns the number of global counters updated.
 * 
 * kevlough: this is an updated version of the function, which
 * checks whether Siloz initialization (memory reservation) has completed.
 * If so, we only update stats for host-reserved nodes.
 */
static int refresh_cpu_vm_stats(bool do_pagesets)
{
	...
	// kevlough: Need to refresh stats on all nodes before Siloz init completes
	if (unlikely(!siloz_init_complete)) {
		return refresh_cpu_vm_stats_no_siloz(do_pagesets);
	}

	// kevlough: now that Siloz init has complete, only update host-reserved nodes
	// TODO: remove hardcoded assumption of 2-socket system
	for (nid = 0; nid < 2; nid++) {
		if (!NODE_DATA(nid)) {
			break;
		}
