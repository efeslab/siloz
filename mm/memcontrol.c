...
static int alloc_mem_cgroup_per_node_info(struct mem_cgroup *memcg, int node)
{
	...
	if (!node_state(node, N_NORMAL_MEMORY))
		tmp = -1;
	pn = kzalloc_node(sizeof(*pn), GFP_KERNEL, node_to_pxm(tmp));
	if (!pn)
		return 1;
...
static int nids[4] = {0,SOCKET_0_EPT_NODE,1,SOCKET_1_EPT_NODE};
...
static int __init mem_cgroup_init(void)
{
	...
	for_each_node(node) {
		...
		rtpn = kzalloc_node(sizeof(*rtpn), GFP_KERNEL,
				    node_online(node) ? node_to_pxm(node) : NUMA_NO_NODE);
