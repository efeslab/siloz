...
static int mmu_topup_memory_caches(struct kvm_vcpu *vcpu, bool maybe_indirect)
{
	...
	if (r)
		return r;
	// kevlough: don't topup EPT pages (minimal set allocated at guest boot)
...
void kvm_mmu_free_shadow_page_cache(struct kvm_vcpu *vcpu)
{
	kvm_mmu_free_memory_cache(&vcpu->arch.mmu_pte_list_desc_cache);
	kvm_mmu_free_memory_cache(&vcpu->arch.mmu_shadow_page_cache);
	kvm_mmu_free_memory_cache(&vcpu->arch.mmu_gfn_array_cache);
...
int kvm_mmu_create(struct kvm_vcpu *vcpu)
{
	...
	// kevlough: quick way to distinguish which cache holds EPTs
	vcpu->arch.mmu_shadow_page_cache.has_sptes = 1;
